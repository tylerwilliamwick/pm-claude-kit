Run this before committing to a plan. CEO/founder-mode review: rethink the problem, find the best version of the product, challenge premises.
...
Your posture depends on what is needed:
...
Critical rule: Once a mode is selected, COMMIT to it. Do not silently drift. If EXPANSION is selected, do not argue for less work during later sections. If REDUCTION is selected, do not sneak scope back in. Raise concerns once in Step 0 — after that, execute the chosen mode faithfully.
...
Before doing anything else, run a system audit to gather the context needed to review the plan intelligently.
...
2. Platonic ideal: If the best engineer in the world had unlimited time and perfect taste, what would this system look like? Start from experience, not architecture.
...
1. **SCOPE EXPANSION:** The plan is good but could be great. Propose the ambitious version, then review that.
2. **HOLD SCOPE:** The plan's scope is right. Review it with maximum rigor — architecture, security, edge cases, observability, deployment.
3. **SCOPE REDUCTION:** The plan is overbuilt or wrong-headed. Propose a minimal version that achieves the core goal.
...
Once selected, commit fully. Ask once per issue. Present a recommendation with WHY, then lettered options. Do NOT proceed until the user responds.
...
* Data flow — all four paths (happy, nil, empty, error) for every new flow.
* State machines — ASCII diagram for every new stateful object. Include impossible/invalid transitions.
* Coupling concerns. Which components are now coupled that weren't before? Draw the before/after dependency graph.
* Scaling characteristics. What breaks first under 10x load? Under 100x?
* Single points of failure. Map them.
* Security architecture. Auth boundaries, data access patterns, API surfaces.
* Production failure scenarios. For each new integration point, describe one realistic failure (timeout, cascade, data corruption, auth failure) and whether the plan accounts for it.
...
Ask once per issue found. Present recommendation + WHY + lettered options. Do NOT proceed to the next section until all issues are resolved.
...
Ask once per issue found. Do NOT proceed until all issues are resolved.
...
For every new user-visible interaction, evaluate edge cases:
```
  ...
  Async operation      | User navigates away    | ?        |
                       | Operation times out    | ?        |
  Background job       | Job fails partway      | ?        |
                       | Job runs twice (dup)   | ?        |
```
...
* DRY violations. If the same logic exists elsewhere, flag it with the file and line reference.
...
* Under-engineering: anything fragile, assuming happy path only, or missing obvious defensive checks?
...
* Ecosystem fit. Aligns with the tech stack's direction?
...
* Escape hatch: If a section has no issues, say so and move on. If an issue has an obvious fix with no real alternatives, state what you'll do and proceed — don't waste a question on it.
...
### TODOS updates
Present each potential TODO as its own individual question. Never batch TODOs — one per question. For each TODO, describe:
...
Then present options: **A)** Add to backlog **B)** Skip **C)** Build it now instead of deferring.

### Delight Opportunities (EXPANSION mode only)
At least 5 "bonus chunk" opportunities (<30 min each) that would make users think "oh nice, they thought of that." Present each as its own question. For each: what it is, why it delights, effort estimate. Options: **A)** Add to backlog **B)** Skip **C)** Build now.
...
**Next step:** After scope is validated, run /plan-eng-review for architecture and feasibility gate.
